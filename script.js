...
const correctRiddleAnswer = "LAYA"; 
const correctRiddleAnswer2 = ["BAIK"];
const correctLockCode = "20042006"; 


const birthdayMessage = [
    "Selamat ulang tahun Laya Dwi Garini!", 
    "Semoga Panjang Umur Sehat Selalu, semoga makin lancar kuliahnya, urusannya, sama hidup kamu juga pastinya!",
    "Dan Semoga di umur kamu yg baru ini, kamu punya semangat yg baru juga ya layy,apapun yg kamu lakuin aku selalu support sebagai temen kamu..",
    "dan semoga kamu selalu dikelilingi orang-orang baik yang bikin kamu nyaman, dan merasa nyaman ada di deket kamu.",
    "Pokoknya semoga makin bahagia, makin tenang, dan makin yakin sama jalan kamu sendiri.",
    "Enjoy ya hari ini, Jangan lupa jaga kesehatan dan jangan lupa senyum hari ini YEYY."
];
...
window.checkAnswer2 = function() {
    ...
    const answer = originalAnswer.toUpperCase(); 
    const correctKeywords = ["BAIK", "ALHAMDULILLAH BAIK", "ALHAMDULILLAH", "SEHAT"];
    const isCorrect = correctKeywords.some(keyword => answer.includes(keyword));

    if (isCorrect) { 
        // --- JAWABAN BENAR --- (Membuat Pita Diam, lalu Transisi Instan)
        message.innerText = `Alhamdulillah...`; 
        message.style.color = '#ff69b4';
        input.disabled = true;
        ...
        setTimeout(() => {
            if (riddleContainer2) { riddleContainer2.style.display = 'none'; }
            if (riddleMainContainer) { riddleMainContainer.style.display = 'none'; }
            goToPage('page3'); 
        }, 1500); 

    } else {
        // --- JAWABAN SALAH --- (Pesan Error yang SANGAT PENDEK)
        
        if (answer.length === 0) {
             message.innerText = "harus diisi ya jgn sampe kosong!"; // Pesan pendek
        } else {
        }
       
        message.style.color = 'Red';
        // Hapus kode goyang (lockshake) jika masih ada
    }
}
window.checkLockCode = function() {
    ...
    if (enteredCode === correctLockCode) {
        ...
        lockMessage.style.color = 'rgb(168, 94, 187)';
        ...
        lockMessage.innerText = "Salahh! Kan Tanggal lahir sendiri masa lupa?";
